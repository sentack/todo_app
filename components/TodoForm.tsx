...
export default function TodoForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log(
        editingTodo
          ? "[v0] Starting todo update..."
          : "[v0] Starting todo submission...",
      );
      ...
      console.log(
        editingTodo
          ? "[v0] Todo updated successfully"
          : "[v0] Todo added successfully",
      );
    } catch (error) {
      console.error(
        editingTodo ? "Error updating todo:" : "Error adding todo:",
        error,
      );
      alert(
        `Failed to ${editingTodo ? "update" : "add"} todo: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    } finally {
      setLoading(false);
    }
        ...
        >
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gray-100 dark:bg-gray-900 group-hover:bg-black dark:group-hover:bg-white flex items-center justify-center transition-all duration-300">
              <svg
                className="w-5 h-5 text-gray-500 dark:text-gray-400 group-hover:text-white dark:group-hover:text-black transition-colors duration-300"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path
            ...
            <div>
              <h3 className="font-semibold text-black dark:text-white group-hover:text-black dark:group-hover:text-white">
                Add new todo
              </h3>
  ...
  return (
    <div className="animate-slide-in-down">
      <form
        ...
        className="space-y-6 p-6 bg-white dark:bg-black border border-gray-200 dark:border-gray-800 rounded-2xl shadow-xl"
      >
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setFormData({
                ...formData,
                status_id: Number.parseInt(e.target.value),
              })
            }

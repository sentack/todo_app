...
interface LongestCompletedTodo {
  id: string
  title: string
  days: number
  hours: number
...
export default function StatsPage() {
  ...
  const [longestCompletedTodos, setLongestCompletedTodos] = useState<LongestCompletedTodo[]>([])
  ...
  const checkAuthAndFetchStats = async () => {
    try {
      ...
      await Promise.all([
        ...
        fetchLongestCompletedTodos(user.id), // Added fetch for longest completed todos
      ])
    } catch (error) {
  ...
  const fetchLongestCompletedTodos = async (userId: string) => {
    const { data } = await supabase
      .from("todos")
      .select("id, title, created_at, updated_at")
      .eq("user_id", userId)
    ...
    if (data) {
      const longest = data
        ...
        .sort((a, b) => b.hours - a.hours) // Sort by longest time first
        .slice(0, 3) // Take top 3

      setLongestCompletedTodos(longest)
    }
  }
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="animate-slide-in-down">
          ...
          <div className="space-y-8">
            ...
            {/* Longest Completed Todos Section */}
            <div>
              <h2 className="text-xl font-semibold text-black dark:text-white mb-6">Longest Completed Todos</h2>
              <div className="bg-white dark:bg-black border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-xl">
                {longestCompletedTodos.length === 0 ? (
                  <div className="text-center py-8">
                    <div className="w-12 h-12 bg-gray-100 dark:bg-gray-900 rounded-xl flex items-center justify-center mx-auto mb-4">
                      <svg className="w-6 h-6 text-gray-400 dark:text-gray-600" fill="currentColor" viewBox="0 0 20 20">
                        <path
                  ...
                  <div className="space-y-4">
                    {longestCompletedTodos.map((todo, index) => (
                      <div
                        key={todo.id}
                        className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-950 rounded-xl animate-slide-in-up"
                        style={{ animationDelay: `${index * 0.1}s` }}
                      >
                        ...
                        <div className="text-right">
                          <div className="text-lg font-bold text-purple-600 dark:text-purple-400">
                            {todo.days > 0 ? `${todo.days}d` : `${todo.hours}h`}
                          </div>
                          <div className="text-xs text-gray-500 dark:text-gray-400">
                            {todo.days > 0 ? "days" : "hours"}
                          </div>
                        </div>
                      </div>
                    ))}

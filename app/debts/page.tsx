...
export default function DebtsPage() {
  ...
  const submitItem = async (e: React.FormEvent) => {
    ...
    if (editingItem) {
      const { data, error } = await supabase.from("debts").update(payload).eq("id", editingItem.id).select().single()
      if (!error && data) { setAllItems(prev => prev.map(i => i.id === data.id ? { ...i, ...data } : i)); closeForm() }
    } else {
      if (!userIdRef.current) { setSubmitting(false); return }
      const { error } = await supabase.from("debts").insert({ user_id: userIdRef.current, ...payload })
      if (!error) { closeForm(); setPage(1); setRefreshKey(k => k + 1) }
    }
    setSubmitting(false)
  }

  const startPay = (item: DebtItem) => {
  ...
  const submitPay = async (e: React.FormEvent, item: DebtItem) => {
    e.preventDefault()
    ...
    const { data, error } = await supabase.from("debts").update({ amount_paid: newAmountPaid, paid_history: newHistory }).eq("id", item.id).select().single()
    if (!error && data) {
        ...
        <div className="animate-slide-in-down">
            ...
            <div className="bg-black dark:bg-white rounded-2xl p-5 mb-6 shadow-xl">
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-white dark:text-black">ETB {summary.paid.toFixed(2)}</p>
                </div>
                <div>
                  <p className="text-gray-400 dark:text-gray-600 text-xs font-medium mb-1">Remaining</p>
                  <p className="text-lg font-bold text-white dark:text-black">ETB {summary.remaining.toFixed(2)}</p>
          ...
          <div className={`overflow-hidden transition-all duration-300 ease-in-out ${formOpen ? "max-h-[500px] opacity-100 mb-6" : "max-h-0 opacity-0"}`}>
            <div className="bg-white dark:bg-black border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-xl">
              <h2 className="text-lg font-semibold text-black dark:text-white mb-4">{editingItem ? "Edit Debt" : "New Debt"}</h2>
              <form onSubmit={submitItem} className="space-y-4">
                <div>
                  <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">Lender (who you owe)</label>
                ...
                <button type="submit" disabled={submitting} className="w-full py-3 bg-black dark:bg-white text-white dark:text-black font-semibold rounded-xl hover:opacity-80 disabled:opacity-50 transition-all">
                  {submitting ? "Saving..." : (editingItem ? "Save Changes" : "Add Debt")}
                </button>
              </form>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-gray-500 dark:text-gray-400">{search || statusFilter !== "all" ? "No debts match your filters." : "No debts yet. Add your first one above."}</p>
            </div>
          ) : (
            <div className="space-y-4">
              {paged.map(item => (
                <DebtCard key={item.id} item={item} payingId={payingId} payAmount={payAmount} payDate={payDate}
...
function DebtCard({ item, payingId, payAmount, payDate, submittingPay, deletingId, onStartPay, onCancelPay, onSetPayAmount, onSetPayDate, onPay, onEdit, onDelete }: {
  item: DebtItem; payingId: string | null; payAmount: string; payDate: string
  submittingPay: boolean; deletingId: string | null
  onStartPay: (i: DebtItem) => void; onCancelPay: () => void
  onSetPayAmount: (v: string) => void; onSetPayDate: (v: string) => void
  onPay: (e: React.FormEvent, i: DebtItem) => void
  onEdit: (i: DebtItem) => void; onDelete: (id: string) => void
}) {
      ...
      <div className="p-5">
        ...
        <div className="mb-4">
          <div className="flex justify-between text-xs text-gray-500 dark:text-gray-400 mb-1.5">
            <span>Paid ETB {Number(item.amount_paid).toFixed(2)}</span>
            <span>{pct}%</span>
          </div>

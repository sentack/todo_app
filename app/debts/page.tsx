...
export default function DebtsPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const [{ data: debts }, { data: lendings }] = await Promise.all([
        supabase.from("debts").select("*").eq("user_id", userIdRef.current!).order("created_at", { ascending: false }),
        supabase.from("lendings").select("*").eq("user_id", userIdRef.current!).order("created_at", { ascending: false }),
      ])
      if (!cancelled) {
  ...
  const groups = useMemo(() => {
    ...
    if (hideSettled) {
      g = g.filter(gr => personNet(gr) !== 0 || gr.debts.some(d => getStatus(d) !== "paid") || gr.lendings.some(l => getStatus(l) !== "paid"))
    }
    return g
    ...
    <AuthenticatedLayout>
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="animate-slide-in-down">
            ...
            <div className="bg-black dark:bg-white rounded-2xl p-5 mb-6 shadow-xl">
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <p className={`text-lg font-bold ${summary.net >= 0 ? "text-green-400 dark:text-green-500" : "text-red-400 dark:text-red-400"}`}>
          ...
          {/* List */}
          {loading ? (
            <div className="space-y-4">{[1, 2, 3].map(i => <div key={i} className="skeleton rounded-2xl h-48" />)}</div>
          ) : groups.length === 0 ? (
...
function PersonGroupCard({ group, currency, userId, onDebtUpdated, onDebtDelete, onLendingUpdated, onLendingDelete }: {
  ...
  const allSettled   = net === 0 && youOweTotal === 0 && theyOweTotal === 0

  return (
    <div className="bg-white dark:bg-black border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden shadow-sm">

      {/* Person header */}
      <div className="flex items-center gap-3 px-4 py-3.5 bg-gray-50 dark:bg-gray-950">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            {[
              group.debts.length > 0   && `${group.debts.length} debt${group.debts.length !== 1 ? "s" : ""}`,
              group.lendings.length > 0 && `${group.lendings.length} lending${group.lendings.length !== 1 ? "s" : ""}`,
            ].filter(Boolean).join(" · ")}
          </p>
        </div>
        <div className="text-right shrink-0">
          {allSettled ? (
            <span className="text-xs font-semibold text-green-500 dark:text-green-400 bg-green-50 dark:bg-green-900/20 px-2.5 py-1 rounded-full">All settled</span>
          ) : (
            <>
              <p className="text-xs text-gray-500 dark:text-gray-400">{net >= 0 ? "You're owed" : "You owe"}</p>
              <p className={`text-base font-bold ${net >= 0 ? "text-green-500 dark:text-green-400" : "text-red-500 dark:text-red-400"}`}>
      ...
      {group.debts.length > 0 && (
        <>
          <div className="flex items-center justify-between px-4 py-2 border-t border-gray-100 dark:border-gray-900 bg-red-50/40 dark:bg-red-950/20">
            <p className="text-xs font-bold text-red-500 dark:text-red-400 uppercase tracking-wider">You Owe</p>
            {youOweTotal > 0 && <p className="text-xs font-semibold text-gray-500 dark:text-gray-400">{currency} {formatMoney(youOweTotal)} remaining</p>}
          </div>
      ...
      {/* Lendings (they owe you) */}
      {group.lendings.length > 0 && (
        <>
          <div className="flex items-center justify-between px-4 py-2 border-t border-gray-100 dark:border-gray-900 bg-green-50/40 dark:bg-green-950/20">
            <p className="text-xs font-bold text-green-600 dark:text-green-400 uppercase tracking-wider">They Owe You</p>
            {theyOweTotal > 0 && <p className="text-xs font-semibold text-gray-500 dark:text-gray-400">{currency} {formatMoney(theyOweTotal)} remaining</p>}
          </div>
          <div className="divide-y divide-gray-100 dark:divide-gray-900">
            {group.lendings.map(l => (
    ...
    <div className="px-4 py-3">
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        <div className="flex items-center gap-1 shrink-0">
          {status !== "paid" && (
            <button
              onClick={() => { setIsPaying(true); setIsEditing(false) }}
          ...
          <form onSubmit={handlePay} className="space-y-2">
            <div className="flex gap-2">
              <div className="flex-1 relative">
                ...
                <input type="number" step="0.01" min="0.01" max={remaining} value={payAmount} onChange={e => setPayAmount(e.target.value)}
                  placeholder={`max ${formatMoney(remaining)}`} required autoFocus className={`${CARD_INPUT} pl-11`} />
              </div>
      ...
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        <div className="flex items-center gap-1 shrink-0">
          {status !== "paid" && (
            <button
              onClick={() => { setIsPaying(true); setIsEditing(false) }}
              className="px-2.5 py-1 text-xs font-semibold bg-green-500 hover:bg-green-600 text-white rounded-lg transition-colors"
            >
              Got Paid
            </button>
          )}

...
export default function LendingPage() {
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      if ((e as CustomEvent).detail?.type === "lending") { setPage(1); setRefreshKey(k => k + 1) }
    }
  ...
  const deleteItem = (id: string) => {
    ...
    let undone = false
    triggerUndo(`Removed lending for ${item.person}`, () => {
      undone = true
        ...
        <div className="animate-slide-in-down">
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-gray-500 dark:text-gray-400">{search || statusFilter !== "all" ? "No lendings match your filters." : "No lendings yet. Use the + button to add one."}</p>
            </div>
          ) : (
            <div className="space-y-4">
              {paged.map(item => (
                <LendingCard key={item.id} item={item} currency={currency}
...
function LendingCard({ item, userId, currency, onUpdated, onDelete }: {
  item: LendingItem; userId: string; currency: string
  onUpdated: (updated: LendingItem) => void; onDelete: (id: string) => void
}) {
  const supabase = useMemo(() => createBrowserSupabaseClient(), [])
